			  ...
			  int cmgoalx, int cmgoalz, int cmgoalminx, int cmgoalminz, int cmgoalmaxx, int cmgoalmaxz)
{
	...
	*/
	...
	while(true)
	{
		...
		if(bestS->cx == 263 && bestS->cz == 144)
			bcorner = true;

		if(bcorner)
			g_log<<"bcorner 263,144"<<endl;
		...
		//if(bestS->previous >= 0 && !PathNode(vBestS, ignoreUnit, ignoreBuilding, roadVehicle))
		...
		*/
		...
		runningD = bestS->totalD;

		if(bestS->cx > 0 && 
			canwest && 
			wt->triedcell[ PathNodeIndex(bestS->cx - 1, bestS->cz) ] != wt->pathcnt)
		{
			PathNode* reachedS = NULL;

			if(bcorner)
				g_log<<"bcorner w"<<endl;
			...
			wt->triedcell[ PathNodeIndex(bestS->cx + 1, bestS->cz) ] != wt->pathcnt)
		{
			...
			if(bcorner)
			{
				g_log<<"bcorner n?"<<endl;

				if(wt->triedcell[ PathNodeIndex(bestS->cx, bestS->cz - 1) ] == wt->pathcnt)
					g_log<<"already visited"<<endl;
