...
void DrawSteps()
{
	...
	for(; siter != wt->trystep.end(); si++, siter++)
	{
		...
		fromvec.x = prevS->cx * PATHNODE_SIZE + PATHNODE_SIZE/2;
		fromvec.z = prevS->cz * PATHNODE_SIZE + PATHNODE_SIZE/2;
		fromvec.y = g_hmap.accheight(fromvec.x, fromvec.z) + TILE_SIZE/200;

		tovec.x = tS->cx * PATHNODE_SIZE + PATHNODE_SIZE/2;
		tovec.z = tS->cz * PATHNODE_SIZE + PATHNODE_SIZE/2;
