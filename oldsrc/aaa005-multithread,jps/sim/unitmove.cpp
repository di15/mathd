...
void MoveUnit(Unit* u)
{
	...
	if(!u->pathblocked && (u->path.size() <= 0 || *u->path.rbegin() != u->goal))
	{
#if 0
		if(t->military)
		{
	...
	if(Magnitude(u->subgoal - u->cmpos) <= t->cmspeed)
	{
		u->cmpos = u->subgoal;
	...
	if(dir.x != 0 || dir.y != 0)
	{
		...
		Vec2i scaleddir = dir * t->cmspeed / Magnitude(dir);
	...
	u->drawpos.z = u->cmpos.y;
#if 0
	u->drawpos.y = Bilerp(&g_hmap, u->drawpos.x, u->drawpos.z);
#else
	u->drawpos.y = g_hmap.accheight(u->drawpos.x, u->drawpos.z);
#endif
}