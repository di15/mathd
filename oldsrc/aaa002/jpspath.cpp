...
bool JPSPath(int wthread, int utype, int umode, int cmstartx, int cmstartz, int target, int targetx, int targetz, 
	...
	//if(startcenterblocked)
	{
		if( cstartx > 0 && cstartz > 0 && cstartx - 1 >= cstartminx && cstartz - 1 >= cstartminz)
		{
			...
			if(CellBlockOpen(cposx, cposz,
						...
						thisu, ignoreu, ignoreb) == COLLIDER_NONE)
			{
				g_log<<"start -1,-1 at "<<cposx<<","<<cposz<<endl;
				g_log.flush();
		...
		if( cstartz > 0 && cstartz - 1 >= cstartminz)
		{
			...
			if(CellBlockOpen(cposx, cposz,
						...
						thisu, ignoreu, ignoreb) == COLLIDER_NONE)
			{
				g_log<<"start 0,-1 at "<<cposx<<","<<cposz<<endl;
				g_log.flush();
				wt->trystep.push_back(TryStep(cstartx, cstartz, cgoalx, cgoalz, cposx, cposz, NULL, 0, 0));
				wt->heap.insert(&*wt->trystep.rbegin());
			}
		...
		if( cstartx > 0 && cstartx - 1 >= cstartminx )
		{
			...
			if(CellBlockOpen(cposx, cposz,
						...
						thisu, ignoreu, ignoreb) == COLLIDER_NONE)
			{
				g_log<<"start -1,0 at "<<cposx<<","<<cposz<<endl;
				g_log.flush();
				wt->trystep.push_back(TryStep(cstartx, cstartz, cgoalx, cgoalz, cposx, cposz, NULL, 0, 0));
				wt->heap.insert(&*wt->trystep.rbegin());
			}
	...
	*/
	...
	while(true)
	{
...
#if 0
		if(searchdepth > (g_pathdim.x + g_pathdim.y)*7)
			return false;
#endif
		...
		do
		{
			...
			if( wt->triedcell[ PathCellIndex(bestS->cx, bestS->cz) ] == wt->pathcnt )
			{
				g_log<<"already visited: "<<bestS->cx<<","<<bestS->cz<<endl;
				g_log.flush();
			}
		...
		{
			ReconstructPathJPS(path, bestS, subgoal, cmgoalx, cmgoalz);
		...
		*/
		...
		{
			TryStep* reachedS = NULL;

			g_log<<"exp w from "<<bestS->cx<<","<<bestS->cz<<endl;
			if(bestS->previous)
			{
				g_log<<"\tfrom exp "<<(int)bestS->expansion<<" from "<<bestS->previous->cx<<","<<bestS->previous->cz<<endl;
			}
			g_log.flush();

			if(Expansion_W(bestS->cx, bestS->cz,
				ut->roaded, ut->landborne, ut->seaborne, ut->airborne,
				thisu, ignoreu, ignoreb,
				ut, wt,
				...
				reachedS))
			{
				ReconstructPathJPS(path, reachedS, subgoal, cmgoalx, cmgoalz);
				return true;
			}
			...
			wt->triedcell[ PathCellIndex(bestS->cx + 1, bestS->cz) ] != wt->pathcnt)
		{
			...
			g_log<<"exp e from "<<bestS->cx<<","<<bestS->cz<<endl;
			if(bestS->previous)
			{
				g_log<<"\tfrom exp "<<(int)bestS->expansion<<" from "<<bestS->previous->cx<<","<<bestS->previous->cz<<endl;
			}
			...
			if(Expansion_E(bestS->cx, bestS->cz,
				ut->roaded, ut->landborne, ut->seaborne, ut->airborne,
				thisu, ignoreu, ignoreb,
				ut, wt,
				cstartx, cstartz, cgoalx, cgoalz,
			...
			wt->triedcell[ PathCellIndex(bestS->cx, bestS->cz - 1) ] != wt->pathcnt)
		{
			...
			g_log<<"exp n from "<<bestS->cx<<","<<bestS->cz<<endl;
			if(bestS->previous)
			{
				g_log<<"\tfrom exp "<<(int)bestS->expansion<<" from "<<bestS->previous->cx<<","<<bestS->previous->cz<<endl;
			}
			...
			if(Expansion_N(bestS->cx, bestS->cz,
				ut->roaded, ut->landborne, ut->seaborne, ut->airborne,
				thisu, ignoreu, ignoreb,
				ut, wt,
				cstartx, cstartz, cgoalx, cgoalz,
			...
			wt->triedcell[ PathCellIndex(bestS->cx, bestS->cz + 1) ] != wt->pathcnt)
		{
			...
			g_log<<"exp s from "<<bestS->cx<<","<<bestS->cz<<endl;
			if(bestS->previous)
			{
				g_log<<"\tfrom exp "<<(int)bestS->expansion<<" from "<<bestS->previous->cx<<","<<bestS->previous->cz<<endl;
			}
			...
			if(Expansion_S(bestS->cx, bestS->cz,
				ut->roaded, ut->landborne, ut->seaborne, ut->airborne,
				thisu, ignoreu, ignoreb,
				ut, wt,
				cstartx, cstartz, cgoalx, cgoalz,
			...
			wt->triedcell[ PathCellIndex(bestS->cx - 1, bestS->cz - 1) ] != wt->pathcnt)
		{
			...
			g_log<<"exp nw from "<<bestS->cx<<","<<bestS->cz<<endl;
			if(bestS->previous)
			{
				g_log<<"\tfrom exp "<<(int)bestS->expansion<<" from "<<bestS->previous->cx<<","<<bestS->previous->cz<<endl;
			}
			...
			if(Expansion_NW(bestS->cx, bestS->cz,
				ut->roaded, ut->landborne, ut->seaborne, ut->airborne,
				thisu, ignoreu, ignoreb, 
				ut, wt, 
				cstartx, cstartz, cgoalx, cgoalz,
				bestS, runningD, cmdiag,
				cmgoalminx, cmgoalminz, cmgoalmaxx, cmgoalmaxz,
				reachedS))
			{
				ReconstructPathJPS(path, reachedS, subgoal, cmgoalx, cmgoalz);
			...
			wt->triedcell[ PathCellIndex(bestS->cx + 1, bestS->cz - 1) ] != wt->pathcnt)
		{
			...
			g_log<<"exp ne from "<<bestS->cx<<","<<bestS->cz<<endl;
			if(bestS->previous)
			{
				g_log<<"\tfrom exp "<<(int)bestS->expansion<<" from "<<bestS->previous->cx<<","<<bestS->previous->cz<<endl;
			}
			...
			if(Expansion_NE(bestS->cx, bestS->cz,
				ut->roaded, ut->landborne, ut->seaborne, ut->airborne,
				thisu, ignoreu, ignoreb, 
				ut, wt, 
				cstartx, cstartz, cgoalx, cgoalz,
			...
			wt->triedcell[ PathCellIndex(bestS->cx - 1, bestS->cz + 1) ] != wt->pathcnt)
		{
			...
			g_log<<"exp sw from "<<bestS->cx<<","<<bestS->cz<<endl;
			if(bestS->previous)
			{
				g_log<<"\tfrom exp "<<(int)bestS->expansion<<" from "<<bestS->previous->cx<<","<<bestS->previous->cz<<endl;
			}
			...
			if(Expansion_SW(bestS->cx, bestS->cz,
				ut->roaded, ut->landborne, ut->seaborne, ut->airborne,
				thisu, ignoreu, ignoreb, 
				ut, wt, 
				cstartx, cstartz, cgoalx, cgoalz,
			...
			wt->triedcell[ PathCellIndex(bestS->cx + 1, bestS->cz + 1) ] != wt->pathcnt)
		{
			...
			g_log<<"exp se from "<<bestS->cx<<","<<bestS->cz<<endl;
			if(bestS->previous)
			{
				g_log<<"\tfrom exp "<<(int)bestS->expansion<<" from "<<bestS->previous->cx<<","<<bestS->previous->cz<<endl;
			}
			...
			if(Expansion_SE(bestS->cx, bestS->cz,
				ut->roaded, ut->landborne, ut->seaborne, ut->airborne,
				thisu, ignoreu, ignoreb, 
				ut, wt, 
				cstartx, cstartz, cgoalx, cgoalz,
