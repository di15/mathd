...
void DrawSteps()
{
	...
	int si = 0;
	auto siter = wt->trystep.begin();
	for(; siter != wt->trystep.end(); si++, siter++)
	{
		TryStep* tS = &*siter;
		...
		fromvec.x = prevS->cx * PATHCELL_SIZE + PATHCELL_SIZE/2;
		fromvec.z = prevS->cz * PATHCELL_SIZE + PATHCELL_SIZE/2;
		fromvec.y = g_hmap.accheight(fromvec.x, fromvec.z) + TILE_SIZE/200;

		tovec.x = tS->cx * PATHCELL_SIZE + PATHCELL_SIZE/2;
		tovec.z = tS->cz * PATHCELL_SIZE + PATHCELL_SIZE/2;
