...
{
	Vec2i npos = PathNodePos(pj->wt, node);
	...
	if(walkable_w)
	{
		...
		PathNode* nextn = PathNodeAt(pj->wt, nextnpos.x, nextnpos.y);
		
		if(!nextn->closed && (!nextn->opened || newD < nextn->totalD))
		{
			pj->wt->toclear.push_back(nextn); // Records this node to reset its properties later.
			nextn->totalD = newD;
			...
			if( !nextn->opened ) 
			{
				pj->wt->openlist.insert(nextn);
				nextn->opened = true;
			}
			else
			{
				pj->wt->openlist.heapify(nextn);
			}
		}
	}
	if(walkable_e)
